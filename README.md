...
**策略**：
- **优先本地规则**（零 LLM 调用）：正则匹配 `有哪些`、`涉及了哪些`、`哪些项目` 等模式
- **本地无法确定时**：调用 LLM 分析 query + 已知实体列表，输出 JSON 决策
...
**降级策略**：LLM 不可用时 → jieba 分词 + 字典匹配 + 本地规则
...
```
LLM 可用 → 完整 pipeline（改写+路由+检索+RAG）
LLM 不可用 → 本地规则（jieba+正则）→ 检索结果降级展示
```
...
│   │   ├── smartRouter.ts    # 语义路由（本地规则 + LLM 决策）
